...
/** INCLUDES *******************************************************/
#include "../Commons/Microchip/USB/USB.h"
#include "../Commons/Microchip/HardwareProfile.h"

#include "../Commons/Microchip/USB/usb_device_local.h"


#if defined(USB_USE_MSD)
    #include "../Commons/Microchip/USB/usb_function_msd.h"
#endif
...
USB_VOLATILE BOOL USBBusIsSuspended; /* Finsun New */
USB_VOLATILE USTAT_FIELDS USTATcopy;
USB_VOLATILE BYTE endpoint_number; /* Finsun new*/
...
volatile BOOL USBDeferStatusStagePacket; /* Finsun new*/
volatile BOOL USBStatusStageEnabledFlag1; /* Finsun new*/
volatile BOOL USBStatusStageEnabledFlag2; /* Finsun new*/
volatile BOOL USBDeferINDataStagePackets; /* Finsun new*/
volatile BOOL USBDeferOUTDataStagePackets; /* Finsun new*/


/** USB FIXED LOCATION VARIABLES ***********************************/
#if defined(__18CXX)
// EDAB: The buffer is at 0x400
...
// Can provide compile time option to do software pingpong
	...
	//#pragma udata
/* Finsun - msd_buffer is defined in usb_config
	#if defined(__18CXX)
...
#endif
{
	...
	/* EDAB: Start-of-frame interrupts only create useless interrupt traffic*/
    if(USBSOFIF)
    {
        if(USBSOFIE)
        {
...
/*	*/
	...
	/* EDAB: Error interrupts are not handled either*/
...
void USBStallEndpoint(BYTE ep, BYTE dir)
{
    ...
    else
    {
        ...
        //then stall that entry as well //modified by Finsun /
...
/*				*/
				// EDAB: Which isn't a problem for PIC18 chips
