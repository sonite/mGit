...
void USBMSDInit(void)
{
    ...
    for(gblCBW.bCBWLUN=0;gblCBW.bCBWLUN<(MAX_LUN + 1);gblCBW.bCBWLUN++)
    {
		...
		// EDAB: Avoid accessing the SD card from here since this function is
		// called from within an interrupt handler. The device will eventually
		// get initialized from the main loop anyway
