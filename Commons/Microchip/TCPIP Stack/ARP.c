...
static void ARPPut(ARP_PACKET* packet);
...
/*****************************************************************************
  ...
  Parameters:
	packet - A pointer to an ARP_PACKET structure with correct operation adn
...
BOOL ARPProcess(void)
{
	// EDAB: Avoid replying to undefined operation codes and strip out pointless state-machine
	ARP_PACKET packet;
...
/*****************************************************************************/
// EDAB: Make an unsolicited ARP announcement to speed up startup
...
void ARPAnnounce(void)
{
	...
	packet.Operation = ARP_OPERATION_RESP;
    memset(&packet.TargetMACAddr, 0, sizeof packet.TargetMACAddr);
    packet.TargetIPAddr = AppConfig.MyIPAddr;
