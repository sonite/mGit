...
{
	return data + TCPPutROMArray(hTCP, data, strlenpgm((far ROM char*)data));
}
#endif
...
static void TCPRAMCopyROM(PTR_BASE wDest, BYTE wDestType, ROM BYTE* wSource, WORD wLength)
{
	...
	{
		case TCP_PIC_RAM:
			memcpypgm2ram((void*)(BYTE*)wDest, (far ROM void*)wSource, wLength);
			break;
		...
		case TCP_ETH_RAM:
			...
			while(wLength)
			{
				...
				// Read and write a chunk	
				memcpypgm2ram(vBuffer, (far ROM void*)wSource, w);
				MACPutArray(vBuffer, w);
				wSource += w;
				wLength -= w;
			}
