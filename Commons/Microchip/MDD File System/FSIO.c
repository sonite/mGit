...
/********************************************************************
* Updated by C.G from 1.2.4 @ 2011-12-27
* Updated by C.G from 1.3.4 @ 2012-05-29
********************************************************************/
...
#include "stdlib.h"
#include "ctype.h"
#include "../Commons/Microchip/MDD File System/FSDefs.h"
...
/*****************************************************************************/
	...
	#pragma udata FileHandles //FINSUN
...
/**************************************************************************
  ...
  **************************************************************************/
// EDAB: Helper function for determining whether a particular file
// (identified based on its first cluster) is open
#ifdef ALLOW_WRITES
static BOOL FILEin_use(const FSFILE *file, BOOL exclusive)
{
	DWORD cluster = file->cluster;
	const BYTE *slot = gFileSlotOpen;
	const FSFILE *array = gFileArray;

	do
	{
		if(!*slot++ && array != file && array->cluster == cluster)
			if(exclusive || file->flags.write)
				return TRUE;
	}
	while(++array != &gFileArray[FS_MAX_FILES_OPEN]);
...
CETYPE FILEopen (FILEOBJ fo, WORD *fHandle, char type)
{
    ...
    dsk = (DISK *)(fo->dsk); //	dsk = (DISK *)(fo->dsk);
    if (dsk->mount == FALSE) //if (gDiskData.mount == FALSE)
    {
        error = CE_NOT_INIT;
    }
    else
    {
        ...
        else
        {
			...
			// EDAB: Force exclusive access when opening a file for writing
#ifdef ALLOW_WRITES
            else if(FILEin_use(fo, fo->flags.write))
            {
				error = CE_FILE_IN_USE;
			}
#endif
            else
            {
...
int SetClockVars (unsigned int year, unsigned char month, unsigned char day, unsigned char hour, unsigned char minute, unsigned char second)
{
	...
	// EDAB: The date has already been validated by the caller
	/*
...
/***************************************************************
  ...
  Remarks:
    None
  ***************************************************************/
/* EDAB: Unused function #ifdef SUPPORT_LFN
...
#ifdef ALLOW_WRITES
// EDAB: Function to erase anything beyond the current file pointer. Explicit used of Log-function
void FSftruncate(FSFILE *stream)
{
	DISK *      disk;
	...
	{
		if(!FAT_erase_cluster_chain(stream->ccls, disk))
			FSerrno = CE_ERASE_FAIL;

		stream->size = stream->seek;
...
/**************************************************************************
  ...
  Remarks:
    None.
  **************************************************************************/
// EDAB: Unused function. Use FSfreadbuf instead
size_t FSfread (void *ptr, size_t size, size_t n, FSFILE *stream)
{
    DWORD   len = size * n;
...
// EDAB:  This is a special version of FSfread which returns a direct pointer
// to the internal read buffer, thus avoiding unnecessary copying.
// Note that this makes 0 the only true error/EOF indicator since the returned
// size will often be a bit short at the end of a sector
size_t FSfreadbuf (void **ptr, size_t len, FSFILE *stream)
{
	DISK    *dsk;               // Disk structure
	DWORD seek;
	DWORD sec_sel;
...
#	endif
	...
	{
		gBufferOwner = stream;
		sec_sel = Cluster2Sector(dsk,stream->ccls); //sec_sel = Cluster2Sector(stream->ccls);
        sec_sel += (WORD)stream->sec;      // add the sector number to it
			
		gBufferZeroed = FALSE;
		if( !MDD_SectorRead( sec_sel, dsk->buffer) ) //if(!MDD_SectorRead(sec_sel, gDataBuffer))
		{
			FSerrno = CE_BAD_SECTOR_READ;
			return 0;
		}
	...
	// Fetch the next sector when the buffer is completely empty
	if(remainder = dsk->sectorSize - pos, !remainder)
	{
		// reset position
		pos = 0;
		// point to the next sector
		...
		if(stream->sec == dsk->SecPerClus)
		{
			...
			if(FILEget_next_cluster(stream, 1) != CE_GOOD)
			{
				FSerrno = CE_COULD_NOT_GET_CLUSTER;
				return 0;
			}
		...
		gLastDataSectorRead = sec_sel;
		remainder = dsk->sectorSize;
	}

	// Limit the read size to the remainder of the sector
	if(len > remainder)
		len = remainder;

	// Have we reached the end of the file?
	if(remainder = stream->size - seek, remainder <= len)
	{
		len = remainder;
		FSerrno = CE_EOF;
	}
	
	// Return the start of the buffer region
	/**ptr = &dsk->buffer[pos]; */ *ptr = &gDataBuffer[pos];

	// Save our new position
	stream->pos = pos + len;
	stream->seek = seek + len;

	// Return the size of whatever block we actually managed to read
	return len;
}
/***************************************************************************
  Function:
...
/*************************************************************
  ...
  Remarks:
    This function is for use with PIC18 when passing arguments in ROM.
  *************************************************************/
/* EDAB: Unused function #ifdef ALLOW_WRITES
int FSremovepgm (const rom char * fileName)
{
	#ifdef SUPPORT_LFN
		char tempArray[257];
...
/**************************************************************
  ...
  Remarks:
    None                                                       
  **************************************************************/
/* EDAB: Unused function - char * FSgetcwd (char * path, int numchars)
{
...
/**************************************************************
  ...
  Remarks:
    None                                                       
  **************************************************************/
/* EDAB: Unused Function - char * wFSgetcwd (unsigned short int * path, int numchars)
{
	char *result;
	twoByteMode = TRUE;
...
#ifdef ALLOW_WRITES
/* EDAB: Unused function int FSmkdir (char * path)
{
