...
********************************************************************
* Updated by C.G from 1.2.4 @ 2011-12-28
* Updated by C.G from 1.3.4 @ 2012-05-28
********************************************************************/
#include <delays.h>
...
{
/* FINSUN: Removed #ifndef MEDIA_SOFT_DETECT
    return(!SD_CD);
#else*/
...
{
    // Turn off the card
    /* FINSUN: Removed SD_CD_TRIS = INPUT;            //Card Detect - input */
    SD_CS = 1;                     //Initialize Chip Select line
    /* FINSUN: Removed SD_CS_TRIS = OUTPUT;           //Card Select - output */
    /* FINSUN: Removed SD_WE_TRIS = INPUT;            //Write Protect - input */
...
BYTE MDD_SDSPI_SectorRead(DWORD sector_addr, BYTE* buffer)
{
    ...
    return FALSE;
/*
		// EDAB: Optimized SPI transfer loop
#		ifndef __18CXX
#			error
#		endif
...
{
    return FALSE; // FINSUN: we dont have -> return(SD_WE);
}
...
void CloseSPIM (void)
{
	...
	SPICON1 &= ~0x20; // FINSUN: not our system? -> SPICON1 &= 0xDF;
...
unsigned char WriteSPIManual(unsigned char data_out)
{
    ...
    SPICLOCKLAT = 0;
    SPIOUTLAT = 1;
    /* FINSUN: Removed SPICLOCK = OUTPUT;
    SPIOUT = OUTPUT; */
...
MEDIA_INFORMATION *  MDD_SDSPI_MediaInitialize(void)
{
	...
	{
		// EDAB: Take the block length into account (READ_BL_LEN @ 83:80)
		/*c_size_mult = (CSDResponse[9] << 1 | CSDResponse[10] >> 7) & 7;
		c_size_mult += CSDResponse[5] & 15;
		c_size_mult += 2 - 9;
		c_size = (DWORD) CSDResponse[6] << 16 | (WORD) CSDResponse[7] << 8 | CSDResponse[8];
		c_size = ((WORD) (c_size >> 6) & 0xFFF) + 1;
		
		MDD_SDSPI_finalLBA = (c_size << c_size_mult) - 1; */
		
		// FINSUN: Why not use the below
		//Must be a v1 device.
