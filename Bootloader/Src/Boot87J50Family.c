...
void UserInit(void)
{
	...
	// EDAB: Blink the memory LED to indicate that we're in the bootloader
	TRISJ &= ~0x04;
}//end UserInit
 ...
 * Note:            None
 *****************************************************************************/
// EDAB: Most-significant byte of the startup timer
unsigned char Boot_Timeout;
static unsigned char Boot_Blink;

// EDAB: Force a stack-overflow reset
void Boot_ResetToApp(void)
{
	// Avoid tail-recursion optimization by calling in assembly language
	_asm rcall Boot_ResetToApp _endasm
}

// EDAB: Keep trying to contact the USB HID bootloader application until
// the timer runs out. Start the application indirectly through a  
// (stack-overflow) reset so as to properly reinitialize the MCU
void Boot_CheckTimeout(void)
{
	...
	// Blink the memory LED based on the timer period
	if((signed char) (TMR0H - Boot_Blink) >= 0)
	{
		Boot_Blink = TMR0H + 0x20;
		LATJbits.LATJ2 ^= 1;
	}

	// Poll the LSB to unlatch the 16-bit counter
	timer = TMR0L;
}
...
void ProcessIO(void)
{
	...
	{	
		switch(PacketFromPC.Command)
		{
			case QUERY_DEVICE:
			{
				// EDAB: Stop USB timeout when the initial query command has been received
				Boot_DisableTimeout();
