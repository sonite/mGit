...
#	ifdef USE_USB_BUS_SENSE_IO
	if(usb_bus_sense != USB_BUS_ATTACHED)       // Is USB bus detached?
	{
...
#	endif
    ...
    if(usb_device_state == ATTACHED_STATE)
    {
        if(!UCONbits.SE0)
        {
			...
			UIE = 0x01 | 					// Enable RESET interrupt
				  0x10;  					// Enable IDLE interrupt
			             					// Mask all other USB interrupts
 ...
 * Note:            None
 *****************************************************************************/
#define USBSoftDetach USBModuleDisable
...
void USBSuspend(void)
{
	...
	//Sleep();				// Go to sleep, wake up when a USB activity event occurs
   	while((PIR2bits.USBIF == 0) && (RCONbits.TO == 0))		//If using the WDT, should go back to sleep if awoke by WDT instead of USBIF
   	{
		...
		// EDAB: We need to check whether the activity timeout has elapsed here as well
		ClrWdt();
		Boot_CheckTimeout();
	}
...
********************************************************************/
	...
	// EDAB: Trigger the long activity timeout now that we've established
